...
def test_ops_incident_hat_reference_scenarios_are_stable():
    data = json.loads(Path("v2/reference/ops_incident_hat_v1/scenarios.json").read_text(encoding="utf-8"))
    ...
    for s in data["scenarios"]:
        ...
        assert out_p.hat == "OPS_INCIDENT_HAT_V1"
        ...
        if out_p.decision != HatDecision.ALLOW:
            assert isinstance(out_p.reasons, list) and len(out_p.reasons) >= 1
            assert all(r.startswith(prefix) for r in out_p.reasons)

        out_c = hat.decide_commit(ctx, proposed, commit)
        assert out_c.hat == "OPS_INCIDENT_HAT_V1"
        ...
        if out_c.decision != HatDecision.ALLOW:
            assert isinstance(out_c.reasons, list) and len(out_c.reasons) >= 1
